...
test.describe("clean-start routes", () => {
  ...
  test("signed-in self onboarding requires report cost confirmation", async ({ page }, testInfo) => {
    ...
    const firstChartButton = page.getByRole("button", { name: "Create My Chart", exact: true });
    await expect(firstChartButton).toBeVisible();
    ...
    await expect(page.getByRole("radio", { name: /Deep Report/ })).toHaveCount(0);
    ...
    await expect(onboarding.getByRole("button", { name: "Create My Chart", exact: true })).toBeVisible();
