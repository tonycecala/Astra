...
test.describe("clean-start routes", () => {
  ...
  test("signed-in self onboarding requires report cost confirmation", async ({ page }, testInfo) => {
    ...
    const kimiIntro = page.getByLabel("Free Introductory Identity Report");
    await expect(kimiIntro).toBeVisible();
    await kimiIntro.check();
    await expect(page.getByRole("radio", { name: /Identity Report/ })).toBeChecked();
    await queueButton.focus();
    await page.keyboard.press("Enter");
    const introductoryConfirmDialog = page.getByRole("dialog", { name: "Confirm Report" });
    await expect(introductoryConfirmDialog).toContainText("Free Introductory Identity Report");
