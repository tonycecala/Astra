...
if (process.argv.includes("--help")) {
  ...
  console.log("Generation: --generate [--output]. Signals only: --signals-only [--output].");
...
try {
  ...
  const signals = buildAstrologyReportSectionEvidence(request, evidenceHeadings);
  const signalValidation = validateSignalPacket(signals);
  const signalMarkdown = renderSignalPacket(signals);
  ...
  if (signalsOnly) {
    await writePrivate(
      ...
      `${JSON.stringify(signalManifest(signalMarkdown, signalValidation), null, 2)}\n`
    );
    ...
    await writePrivate(
      ...
      `${JSON.stringify(
        {
          ...signalManifest(signalMarkdown, signalValidation),
          model,
...
function renderSignalPacket(signals: SignalPacket) {
  return [
...
function buildPrompt(signalMarkdown: string) {
...
function signalManifest(signalMarkdown: string, signalValidation: ReturnType<typeof validateSignalPacket>) {
  return {
    experiment: "dual-perspective-synastry-v3-direct-chart-signals",
    generatedAt,
    ...
    signalSource: "buildAstrologyReportSectionEvidence",
...
function validateSignalPacket(signals: SignalPacket) {
  const issues: string[] = [];
  ...
  if (JSON.stringify(headings) !== JSON.stringify(evidenceHeadings)) {
...
function validatePortrait(markdown: string, signals?: SignalPacket) {
