...
const unnecessaryOrbPrecisionPattern = /\b(?:orb(?:\s+of)?|close and exact|(?:aspect|trine|square|opposition|sextile|conjunction|quincunx)\s+(?:is\s+)?exact|exact\s+(?:aspect|trine|square|opposition|sextile|conjunction|quincunx)|(?:under|within|nearly|less than|about)\s+(?:one|\d+(?:\.\d+)?)\s+degrees?|degrees?\s+(?:apart|from exact))\b|\b(?:aspect|conjunct(?:ion)?|oppos(?:es|ition)|squar(?:e|es)|trin(?:e|es)|sextil(?:e|es)|quincunx(?:es)?)\b[^.!?]{0,160}\b(?:angular distance|tightness|closeness|exactness|intensity|precision|measurement)\b|\b(?:angular distance|tightness|closeness|exactness|precision|measurement)\b[^.!?]{0,160}\b(?:aspect|conjunct(?:ion)?|oppos(?:es|ition)|squar(?:e|es)|trin(?:e|es)|sextil(?:e|es)|quincunx(?:es)?)\b/i;
const uranusNeptuneConjunctionPattern = /\b(?:Uranus\s+(?:(?:is\s+)?conjunct\s+(?:to\s+|with\s+)?|(?:is\s+in|forms?)\s+(?:a\s+)?conjunction\s+with\s+|conjunction\s+)Neptune|Uranus[\s–—-]+Neptune\s+conjunction|(?:conjunction|aspect)\s+between\s+Uranus\s+and\s+Neptune)\b/i;
...
const outputDir = join(packetDir, "blockers", "marissa-gifts-orb-wording");
...
try {
  const bundle = await exportPortableUserData(db, {
    email,
    sourceLabel: "semantic-synthesis-v2-phase-5-marissa-gifts-orb-control"
  });
  ...
  const request = astrologyReportRequestSchema.parse({
    ...
    intent: "Semantic Synthesis V2 Phase 5.1 targeted Marissa Gifts orb-wording control.",
    context: {
      relationshipContext: {
        status: "unspecified",
  ...
  const aspectEvidence = giftsEvidence.evidenceBullets
    .find((bullet) => bullet.label === "uranus conjunction neptune");
  const aspectEvidencePass = Boolean(
    aspectEvidence &&
    /\baspect conjunction\b/.test(aspectEvidence.meaning) &&
    !/\borb\b|\b\d+(?:\.\d+)?\s+degrees?\b/i.test(aspectEvidence.meaning)
  );
  if (!aspectEvidencePass) {
    throw new Error(`Aspect evidence lost its relationship or retained unnecessary orb precision: ${JSON.stringify(aspectEvidence)}`);
  }
  ...
  await writePrivate(
    ...
    [
      "# Marissa Gifts unnecessary orb-wording blocker",
      "",
      `Decision: ${decision}`,
      "",
      ...
      `- Aspect relationship retained in evidence: ${aspectEvidencePass ? "PASS" : "FAIL"}`,
      `- Orb measurement omitted from evidence: ${aspectEvidencePass ? "PASS" : "FAIL"}`,
      `- Aspect relationship retained in prose: ${acceptedBody && uranusNeptuneConjunctionPattern.test(acceptedBody) ? "PASS" : "FAIL"}`,
      `- Orb precision omitted from prose: ${acceptedBody && !unnecessaryOrbPrecisionPattern.test(acceptedBody) ? "PASS" : "FAIL"}`,
      ...
      "",
      decision === "PASS"
        ? "The targeted chapter retained the Uranus-Neptune conjunction while omitting its orb measurement and exactness language."
...
function buildPrompt(
  ...
  return [
    ...
    "A dispositor is a rulership relationship, never an aspect. Do not call a dispositor a conjunction, opposition, square, trine, sextile, or quincunx.",
    "Mention an aspect only when the selected evidence explicitly labels that exact relationship as an aspect.",
    "Name the selected Uranus-Neptune conjunction once as a plain aspect fact. Do not discuss or disclaim its measurement, angular distance, tightness, closeness, exactness, intensity, or precision.",
...
function validateChapter(
  ...
  if (unnecessaryOrbPrecisionPattern.test(body)) issues.push("The chapter states unnecessary orb precision or aspect exactness.");
  if (!uranusNeptuneConjunctionPattern.test(body)) issues.push("The chapter omits the selected Uranus-Neptune conjunction.");
