...
const venusRulesFifthPattern = /\bVenus\b.{0,50}\b(?:rules|ruling)\s+(?:your\s+|the\s+)?(?:5th|fifth)\s+house\b/i;
const plutoDisposedByMarsPattern = /\bPluto\b.{0,50}\b(?:is\s+)?disposed\s+by\s+Mars\b/i;
const uranusNatalRelevancePattern = /\b(?:Uranus\s+has\s+natal relevance|natal relevance\s+(?:of|for)\s+Uranus)\b/i;
const personalActivationOverreachPattern = /\b(?:personal activation|natal relevance)\b[\s\S]{0,300}\b(?:unpredictab(?:ility|le)|inspir(?:ation|ed)|clarif(?:y|ies|ied|ying|ication)|destabili(?:ze|zes|zed|zing|zation)|current timing|currently|right now|this season|makes? you|means? you|shows? that you|you (?:tend to|usually|always|become|act|react))\b|\b(?:unpredictab(?:ility|le)|inspir(?:ation|ed)|clarif(?:y|ies|ied|ying|ication)|destabili(?:ze|zes|zed|zing|zation))\b[\s\S]{0,220}\b(?:personal activation|natal relevance)\b/i;
...
const outputDir = join(packetDir, "blockers", "marissa-gifts-personal-activation");
...
try {
  const bundle = await exportPortableUserData(db, {
    email,
    sourceLabel: "semantic-synthesis-v2-phase-5-marissa-gifts-personal-activation-control"
  });
  ...
  const request = astrologyReportRequestSchema.parse({
    ...
    intent: "Semantic Synthesis V2 Phase 5.1 targeted Marissa Gifts personal-activation control.",
    context: {
      relationshipContext: {
        status: "unspecified",
  ...
  const activationEvidence = giftsEvidence.evidenceBullets
    .find((bullet) => bullet.label === "uranus personal activation");
  const activationEvidencePass = Boolean(
    activationEvidence &&
    /\bUranus has natal relevance\b/.test(activationEvidence.meaning) &&
    /\bnatal relevance only\b/.test(activationEvidence.meaning) &&
    /\bpersonal activation\b/.test(activationEvidence.meaning)
  );
  if (!activationEvidencePass) {
    throw new Error(`Uranus personal activation is not bounded to natal relevance: ${JSON.stringify(activationEvidence)}`);
  }
  ...
  await writePrivate(
    ...
    [
      "# Marissa Gifts personal-activation overreach blocker",
      "",
      `Decision: ${decision}`,
      "",
      ...
      `- Uranus activation retained as natal relevance in evidence: ${activationEvidencePass ? "PASS" : "FAIL"}`,
      `- Uranus activation retained as natal relevance in prose: ${acceptedBody && uranusNatalRelevancePattern.test(acceptedBody) ? "PASS" : "FAIL"}`,
      `- Unsupported activation qualities, effects, timing, and behavior omitted: ${acceptedBody && !personalActivationOverreachPattern.test(acceptedBody) ? "PASS" : "FAIL"}`,
      ...
      "",
      decision === "PASS"
        ? "The targeted chapter retained Uranus personal activation as natal relevance without turning it into qualities, effects, timing, or established behavior."
...
function buildPrompt(
  ...
  return [
    ...
    'Use this exact complete sentence once: "Uranus has natal relevance."',
    "Add no other sentence, explanation, interpretation, denial, disclaimer, or example about Uranus or personal activation.",
...
function validateChapter(
  ...
  if (!uranusNatalRelevancePattern.test(body)) issues.push("The chapter does not preserve Uranus personal activation as natal relevance.");
  if (personalActivationOverreachPattern.test(body)) issues.push("The chapter turns Uranus natal relevance into unsupported qualities, effects, timing, or behavior.");
