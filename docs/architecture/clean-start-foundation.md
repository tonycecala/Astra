...
Astra consumes public fallback/source-layer Composer artifacts through `/api/composer/stream-artifacts`, guarded by `x-astra-internal-token`. The route validates `composerStreamArtifactSchema` and persists the card and stream item through `upsertComposerStreamArtifact`. Astra does not import Composer app code for this handoff. Signed-in Journey cards now use the private `UserFeedItem` read model instead of treating this public fallback stream as the product feed.
...
Generated reports can publish their explicit signal through `/api/reports/[requestId]/publish-signal`. The route authorizes ownership, requires a completed result with `publicSignal`, builds a `ComposerPrivateFeedWrite`, and writes a user-owned `report_signal` feed projection through the shared private-feed service. Raw report sections, provenance detail, and engine payloads remain private in `/self` and the report result tables.

Composer can publish report-derived private feed cards through `ComposerPrivateFeedWrite`, which accepts the explicit `AstrologyReportPublicSignal` boundary, optional source-card material, a user-owned feed projection, and private decision metadata. Run `npm run test:astrology-engine` to prove both unconfigured failure and configured local engine completion, including Tony's Gemini/Virgo/Cancer fixture, Einstein's public AA Pisces/Sagittarius/Cancer fixture, deterministic writer provenance, and unsupported-writer fail-closed behavior. Run `npm run test:report-api` with the local app, migrated database, Mailpit, and app-local env loaded to prove authenticated report create/list, user-owned generation, missing-token rejection, wrong-token rejection, completed internal result recording, non-LLM writer evidence, public-signal preservation, and private feed publish behavior.
