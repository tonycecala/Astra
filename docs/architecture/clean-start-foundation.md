...
- `packages/astrology` owns the astrology report generation boundary. It fails explicitly while no ephemeris engine is configured and can run the first local `astronomy-engine` backed adapter with `ASTRA_EPHEMERIS_ENGINE=local-astronomy-engine`.
...
`@astra/astrology` is now a real module boundary. Without `ASTRA_EPHEMERIS_ENGINE`, it returns a failed `RecordAstrologyReportResult` that records the accepted request and explicit engine-unavailable provenance. With `ASTRA_EPHEMERIS_ENGINE=local-astronomy-engine`, it uses the MIT `astronomy-engine` package to calculate Sun, Moon, visible-planet ecliptic longitudes, and moon phase, then emits a conservative completed report result with private sections/provenance and an explicit public signal for Composer.
...
Composer can publish report-derived stream cards through `publishAstrologyReportSignalArtifact`, which accepts `AstrologyReportPublicSignal` and emits the same `ComposerStreamArtifact` contract Astra already ingests. Run `npm run test:astrology-engine` to prove both unconfigured failure and configured local engine completion. Run `npm run test:report-api` with the local app, migrated database, Mailpit, and app-local env loaded to prove authenticated report create/list, user-owned generation, missing-token rejection, wrong-token rejection, completed internal result recording, and public-signal preservation.
