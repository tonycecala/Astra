...
| AS-R-002 | medium | Contracts | Report/chart/generation contracts, including per-family writer policy and prompt-layer scope, can drift across packages, routes, and hosted configuration. | `packages/contracts/`, report APIs, model strategy, Core/Deep/Progressed Evidence-to-Prose fixtures | Update contracts/docs/tests together; regression-test every prompt shape for each approved family and explicit exclusions; preserve family-specific provenance; keep a global hosted writer override unset unless an owner explicitly approves a temporary replay. | open |
