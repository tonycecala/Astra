...
| AS-R-002 | medium | Contracts | Report/chart/generation contracts, including per-family writer policy, can drift across packages, routes, and hosted configuration. | `packages/contracts/`, report APIs, model strategy | Update contracts/docs/tests together; name and regression-test every approved per-family writer route; keep a global hosted writer override unset unless an owner explicitly approves a temporary replay. | open |
