...
## 2026-08-02 — Focus-first onboarding product model

- Implemented the approved four-step customer onboarding with an optional private focus question, mutable `app_user_profiles.metadata.explorerFocus`, immutable chart-request intent/question/context snapshots, and focus-aware deterministic and Composer envelopes. No database migration was added.
- Chart Arrival remains the success moment. `Enter Astra` now atomically completes Arrival and onboarding and upserts exactly one deterministic `focus_first_exploration` Journey step without creating reports, artifacts, credit events, or retired `composer_onboarding_card` items. Retry preserves prior Journey state; admin and operator accounts create no starter.
- Added prospective focus editing on Self, exact-chart Journey CTA routing, and explainable `Why this now?` copy. All interface chrome is i18n-driven; the optional question stays out of Arrival, Journey, provenance, analytics, logs, and public projections.
- Polished the Journey action hierarchy after rendered review: Explore uses the Astra-gold primary treatment, Complete and Save use padded dark actions, and Dismiss uses a quieter gold-outline treatment.
- Contract/copy evals passed for five focus variants plus skip; typecheck, lint (one unrelated existing warning), i18n/boundary/foundation checks, production build, the eight-test desktop Journey suite, role/privacy coverage, and the complete authenticated path at 1440×900, 820×1180, and 390×844 all passed. Rendered review evidence lives in `docs/review/focus-first-onboarding/`.
- Steward review: **accept**. The implementation follows Time to Value, Discovery, Personalisation, Progressive Disclosure, Setup Defaults, Empty States, Success Moments, Commitment, Intent Mirroring, Fail Safe, JTBD Copywriting, and Trust Building. Logged-out behavior is unchanged; runtime analytics remain N/A until a transport exists; no dark pattern or public/private boundary regression was found.
